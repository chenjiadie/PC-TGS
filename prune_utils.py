...
# def filter_by_mahalanobis(mu_2d, cov_2d, tau=3.0):
#     """
#     mu_2d:  (N, M, 2)
#     cov_2d: (N, M, 2, 2)
#     tau:    标量阈值
#     返回: (mu_2d_new, cov_2d_new)，形状与原始一致，
#          每个 batch 的第二维将有效值排前，不满足条件的补零在末尾
#     """

#     N, M, _ = mu_2d.shape
#     cov2d_inv = torch.linalg.inv(cov_2d)  # (N, M, 2, 2)

#     diff_col = mu_2d.unsqueeze(-1)  # (N, M, 2, 1)
#     left_term = torch.matmul(cov2d_inv, diff_col)  # (N, M, 2, 1)
#     D2 = torch.matmul(diff_col.transpose(-1, -2), left_term).squeeze(-1).squeeze(-1)  # (N, M)

#     mask = D2 < tau**2  # (N, M) 布尔

#     mu_new_list = []
#     cov_new_list = []

#     for i in range(N):
#         valid_mu = mu_2d[i][mask[i]]  # (K, 2)
#         valid_cov = cov_2d[i][mask[i]]  # (K, 2, 2)
#         K = valid_mu.shape[0]

#         # 创建零张量并填充前K个位置
#         mu_padded = torch.zeros(M, 2, dtype=mu_2d.dtype, device=mu_2d.device)
#         cov_padded = torch.zeros(M, 2, 2, dtype=cov_2d.dtype, device=cov_2d.device)
#         mu_padded[:K] = valid_mu
#         cov_padded[:K] = valid_cov

#         mu_new_list.append(mu_padded)
#         cov_new_list.append(cov_padded)

#     mu_2d_new = torch.stack(mu_new_list, dim=0)  # (N, M, 2)
#     cov_2d_new = torch.stack(cov_new_list, dim=0)  # (N, M, 2, 2)

    # return mu_2d_new, cov_2d_new

# def filter_by_mahalanobis(mu_2d, cov_2d, tau=3.0):
#     """
#     并行版本：在 M 维上有效值排前，无效值排后并补零
#     """
#     N, M, _ = mu_2d.shape
...
#     # 获取排序索引，使得 mask=1 的排前，mask=0 的排后
...
#     mask_sorted = mask[batch_idx, sort_idx].unsqueeze(-1)  # (N, M, 1)

#     # 末尾补零
#     mu_2d_new = mu_sorted * mask_sorted
#     cov_2d_new = cov_sorted * mask_sorted.unsqueeze(-1)
...
def filter_by_mahalanobis(mu_2d, cov_2d,sorted_weight, tau=3.0, eps=1e-6):
    ...
    # mask_sorted1 = mask[batch_idx, sort_idx]
    ...
    # return mu_2d_new, cov_2d_new,weight_new 
    return weight_new 
...
if __name__ == "__main__":
    ...
    # print("mu_2d_new shape:", mu_new.shape)  # (N, M, 2)
    # print("cov_2d_new shape:", cov_new.shape)  # (N, M, 2, 2)
